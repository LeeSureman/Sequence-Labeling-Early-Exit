...
import sys
sys.path.append('..')
from V0.load_data_ import load_ontonotes,load_conll,load_ontonotes_cn,load_conll_two_col
import torch
...
# a.unknown_idx
import fastNLP
from fastNLP.embeddings import BertEmbedding,RobertaEmbedding
...
parser.add_argument('--vit_log_softmax',type=int,default=0)

parser.add_argument('--train_mode',choices=['one_cls','joint'],default='joint')
...
parser.add_argument('--ptm_name',default='hfl/chinese-bert-wwm',
                    choices=['albert-base-v2','hfl/chinese-bert-wwm','bert-base-cased',
                             'hfl/chinese-roberta-wwm-ext']) #适用于transformers，而不是FastNLP
...
cn_ptm = ['hfl/chinese-bert-wwm','voidful/albert_chinese_base']
language_ = 'none'
...
if args.fast_ptm_name != '':
    if args.fast_ptm_name == 'bert':
        ...
        if language_ == 'en':
            ...
            args.ptm_name = 'hfl/chinese-roberta-wwm-ext'
        ...
        if language_ == 'en':
            ...
            args.ptm_name = 'voidful/albert_chinese_base'
...
refresh_data = False
print('ptm_name:{}'.format(args.ptm_name))
cache_name = 'cache/{}_{}_{}_2'.format(args.dataset,args.encoding_type,args.ptm_name)
...
from V0.modeling_bert_ee import BERT_SeqLabel_EE
from V0.modeling_albert_ee import Albert_SeqLabel_EE

if 'albert' in args.ptm_name:
    model = Albert_SeqLabel_EE(bundle,args)
else:
...
# from fastNLP import
from fastNLP.core.metrics import _bioes_tag_to_spans