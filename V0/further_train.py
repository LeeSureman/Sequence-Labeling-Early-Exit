...
parser.add_argument('--exp_path',default='01_24_12_27_03.563835')
parser.add_argument('--ckpt_epoch',type=int,default=12)
parser.add_argument('--device',default='0')
parser.add_argument('--flooding',type=int,default=1)
parser.add_argument('--flooding_bias',type=float,default=0)
...
parser.add_argument('--msg',default='之前新加的label embed没有优化器')
...
parser.add_argument('--train_mode',choices=['one_cls','joint','joint_sample_copy'],default='joint_sample_copy')
parser.add_argument('--test_mode',choices=['one_cls','joint','s_level_ee','t_level_ee','t_level_ee_copy'],default='joint')
parser.add_argument('--copy_supervised',type=int,default=0)
parser.add_argument('--copy_wordpiece',choices=['all','first'],default='all')
parser.add_argument('--if_random',type=int,default=0)
...
parser.add_argument('--max_threshold_2',type=float,default=1000)
...
parser.add_argument('--gradient_clip_norm_other',type=float,default=5)
...
model_args.flooding_bias = args.flooding_bias
...
model_args.copy_wordpiece = args.copy_wordpiece
...
bundle = torch.load(open(bundle_path,'rb'))
print('dataset:{}'.format(model_args.dataset))
...
print('after append fitlog callback')
# callbacks.append(FitlogCallback())
import torch.nn as nn
...
# print('token_exit_layers_num:{}'.format(token_exit_layers_num))
print('before test joint')
if args.test_joint:
    tester = Tester(bundle.datasets['test'],model,metrics,16,device=device,verbose=True)
    tester.test()

# model_args.train_mode =
if args.train_mode == 'joint_sample_copy' or args.train_mode == 'joint_sample_copy_label':
    ...
    sample_stop_update_callback = Sample_Stop_Update_Callback('t_level_win_ee',args.sandwich_small,args.sandwich_full,device,constrain_both,12,args.true_copy,
                                                              ...
                                                              max_threshold_2=args.max_threshold_2,
                                                              min_t_level_t=args.min_t_level_t,
                                                              max_t_level_t=args.max_t_level_t,
                                                              copy_wordpiece=args.copy_wordpiece,
                                                              if_random=args.if_random)
...
trainer = Trainer(bundle.datasets['train'],model,optimizer,LossInForward(),args.batch_size,sampler,
                  n_epochs=args.epoch,dev_data=bundle.datasets['dev'],metrics=metrics,
                  metric_key='f' if model_args.dataset not in english_pos_dataset else None,
